  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Fallback resilience: NEVER leave patient with only error message!
        const fallbackMsg: ChatMessageItem = {
          id: (Date.now() + 1).toString(),
          role: 'assistant',
          content: `I understand you are feeling unwell (${userQuery}). Here is safe temporary care guidance:\n\n• Rest in a safe, comfortable, and well-ventilated position.\n• Sip clean water to stay hydrated.\n• Avoid driving, operating machinery, or strenuous activity.\n• Please connect with your assigned ASHA worker or visit the nearest health facility below for evaluation.`,
      ...
      const offlineMsg: ChatMessageItem = {
        id: (Date.now() + 1).toString(),
        role: 'assistant',
        content: `I logged your health concern (${userQuery}). Temporary care guidance:\n\n• Rest quietly and stay hydrated.\n• Do not take unprescribed medications.\n• Connect directly with your ASHA field worker or visit the nearest clinic below.`,
        is_health_concern: true,
        severity: 'warning',
        asha_worker: {
          name: 'Priya Sharma (ASHA)',
