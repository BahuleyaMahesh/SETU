...
export const StaffPatientChat: React.FC = () => {
  ...
  const handleSend = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!input.trim() || loading || !selectedPatientId) return;
    ...
    try {
      const response = await fetch('/api/v1/chat/staff-message', {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ patient_id: selectedPatientId, message: userQuery }),
      });
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-t-2xl p-4 flex items-center justify-between shadow-sm gap-3 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">Report a patient's symptoms — deterministic rules decide risk, never the AI</p>
          </div>
        </div>
      ...
      <div className="flex-1 bg-slate-50/90 dark:bg-slate-900/90 border-x border-slate-200 dark:border-slate-800 p-4 overflow-y-auto space-y-6">
        {!selectedPatientId ? (
          <div className="h-full flex flex-col items-center justify-center text-center text-slate-400 dark:text-slate-500 gap-2">
            ...
            <p className="text-sm font-medium">Select a patient above to start reporting symptoms or asking about their care.</p>
          </div>
        ) : (
          <>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-b-2xl p-3 shadow-sm">
        <form onSubmit={handleSend} className="flex gap-2">
          <input
            ...
            placeholder={selectedPatientId ? "Describe the patient's symptoms or condition…" : 'Select a patient first…'}
          ...
          <Button
            type="submit"
            disabled={loading || !input.trim() || !selectedPatientId}
