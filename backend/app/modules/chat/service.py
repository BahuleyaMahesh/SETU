...
class ChatService:
    ...
    # Cap on how many at-risk patients get their full clinical profile (symptoms
    # + risk reasons) pulled for a roster query — each is a handful of extra
    # queries via ClinicalPipelineService, so this bounds cost/latency on a
    # large roster. The summary line (name/village/risk/last check-in) is
    # still included for every patient regardless of this cap.
    ROSTER_DETAIL_LIMIT = 20

    async def build_roster_summary(self, user: User) -> Dict[str, Any]:
        """Fetch the FULL patient roster this staff member is authorized to
        see, and format it as compact text for an LLM to answer questions
        against. Reuses the exact same scoped queries that already power
        `GET /api/v1/patients` (PatientService.get_patients_by_hospital` /
        `get_patients_by_asha`) — an ASHA only ever gets their own assigned
        patients, a hospital user only ever gets their own hospital's
        patients, same as every other view in this app. This is what makes
        "the whole database" safe to hand to the chatbot: it's already
        exactly the subset that role can see everywhere else, not a
        ...
        if user.role == "hospital":
            if not user.hospital_id:
                return {"error": "No hospital linked to this account"}
            roster = await patient_service.get_patients_by_hospital(str(user.hospital_id))
        elif user.role == "asha":
            if not user.asha_worker_id:
                return {"error": "No ASHA profile linked to this account"}
            roster = await patient_service.get_patients_by_asha(str(user.asha_worker_id))
        else:
            return {"error": "Access denied"}

        if not roster:
            return {"context": "This caseload currently has no patients.", "patient_count": 0}

        from ..clinical.service import ClinicalPipelineService
        clinical_service = ClinicalPipelineService(self.db)

        lines = [f"Total patients: {len(roster)}\n"]
        detail_budget = self.ROSTER_DETAIL_LIMIT
        for p in roster:
            risk = (p.get("risk_level") or "normal").upper()
            # last_checkin is a naive-UTC isoformat string straight off the
            # DB column — format_local converts it to the IST a human
            # reading this roster (and an ASHA/hospital user asking about
            # it) actually means, same fix as the reminder-time bug.
            last_checkin = "never"
            if p.get("last_checkin"):
                try:
                    last_checkin = format_local(datetime.fromisoformat(p["last_checkin"]))
                except ValueError:
                    last_checkin = p["last_checkin"]
            # dict.get(key, default) only falls back on a MISSING key, not a
            # present key whose value is None — every patient here has a
            # 'gender' key (from PatientService.get_patients), so a genuinely
            # ungendered record has value None, not a missing key, and
            # `.get('gender', '')[:1]` crashed with "'NoneType' object is
            # not subscriptable" on the very first real hospital-role query.
            gender_initial = (p.get("gender") or "")[:1]
            line = (
                f"- {p['full_name']} (MRN {p['mrn']}, {p.get('age') or '?'}{gender_initial}) "
                f"— Village: {p.get('village') or 'N/A'} — Risk: {risk} "
                f"— Last check-in: {last_checkin} — Phone: {p.get('phone') or 'N/A'}"
            )

            # Pull real symptoms/risk-reasons for at-risk patients only — the
            # question a staff member actually asks ("why is X critical",
            # "what's wrong with X") almost always concerns someone who
            # isn't 'normal', and a normal patient has nothing to explain.
            if risk != "NORMAL" and detail_budget > 0:
                try:
                    profile = await clinical_service.get_patient_clinical_profile(p["id"])
                    detail_budget -= 1
                    reasons = ((profile or {}).get("latest_risk") or {}).get("risk_reasons") or []
                    symptoms = [s.get("symptom_name") or s.get("symptom_key")
                                for s in (profile or {}).get("combined_symptoms") or []]
                    if reasons:
                        line += f"\n    Reasons: {'; '.join(reasons)}"
                    if symptoms:
                        line += f"\n    Reported symptoms: {', '.join(symptoms)}"
                except Exception:
                    pass  # missing detail for one patient shouldn't break the whole roster

            lines.append(line)

        return {"context": "\n".join(lines), "patient_count": len(roster)}

    async def process_roster_query(
        self,
        user: User,
        user_message: str,
        conversation_id: Optional[str] = None,
    ) -> Dict[str, Any]:
        """Answer a free-text question against the caller's WHOLE authorized
        patient roster — "which of my patients are critical", "when did
        Ramesh last check in", "who's in Whitefield" — instead of requiring
        a single patient to be picked first (process_staff_message above).
        Read-only: this never writes a checkin/alert/risk record, unlike the
        per-patient flow, because a general question isn't a symptom report.
        """
        roster = await self.build_roster_summary(user)
        if roster.get("error"):
            return roster
        ...
        if self.gemini.available():
            try:
                response_text = await self.gemini.answer_roster_query(
                    user_message, roster["context"], user.role
                )
            except Exception as e:
                print(f"[roster-query] Gemini error: {e}")

        if not response_text or not response_text.strip():
            # Deterministic, still-useful fallback when Gemini is unavailable
            # or errors — never a fake/misleading answer, just the real
            # counts computed directly from the same roster data.
            response_text = self._fallback_roster_summary(roster["context"], roster["patient_count"])
        ...
        return {
            ...
            "sources": ["SETU Patient Roster (live)"],
            "timestamp": datetime.utcnow().isoformat(),
        }

    def _fallback_roster_summary(self, roster_context: str, patient_count: int) -> str:
        """Plain, honest fallback when Gemini isn't available to phrase a
        real answer — reports true counts pulled from the roster text
        itself rather than pretending to answer the free-text question."""
        critical = roster_context.count("Risk: CRITICAL")
        warning = roster_context.count("Risk: WARNING")
        normal = patient_count - critical - warning
        return (
            f"AI phrasing isn't available right now, but here is your current caseload: "
            f"{patient_count} total patients — {critical} critical, {warning} warning, {normal} normal. "
            "Open the Patients list to search or filter by name."
        )
