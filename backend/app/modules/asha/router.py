...
async def add_asha_patient(
    ...
    """Add a new patient by an authenticated ASHA worker, or by hospital/admin
    staff (patient created unassigned — no ASHA yet — matching the existing
    "Unassign / reassign" pattern already used elsewhere in the roster UI)."""
    if current_user.role not in ("asha", "hospital", "admin"):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only ASHA workers or hospital/admin staff can add patients",
        )
    if current_user.role == "asha" and not current_user.asha_worker_id:
        raise HTTPException(
