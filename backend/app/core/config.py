...
class Settings(BaseSettings):
    ...
    SMTP_FROM_NAME: str = os.getenv("SMTP_FROM_NAME", "SETU Care Team")
    ...
    # Local wall-clock timezone of the people this platform serves, as
    # minutes offset from UTC. Defaults to IST (UTC+5:30) — this is a rural
    # India deployment. Every timestamp in the DB is stored as naive UTC, so
    # anything expressed as a HUMAN clock time ("take this at 8 in the
    # morning", "call me at 6:30pm") has to be converted through this offset
    # before it's stored, or it fires 5.5 hours off. See core/timeutils.py.
    LOCAL_UTC_OFFSET_MINUTES: int = int(os.getenv("LOCAL_UTC_OFFSET_MINUTES", "330"))
